...
public class AnimTask {
    ...
    private List<UpdateInfo> getUpdateList(Object obj) {
        ...
        List<UpdateInfo> list2 = (List) ObjectPool.acquire(ArrayList.class, new Object[0]);
    ...
    private void notifyUpdateAndEnd(Collection<AnimRunningInfo> collection) {
        ...
        for (Object obj : list) {
            ObjectPool.release(this.mUpdateMap.remove(obj));
        }
        ObjectPool.release(list);
    }
        ...
        for (AnimRunningInfo animRunningInfo : list) {
            if (animRunningInfo.isRunning()) {
                ...
                if (isAnimEnd || (animRunningInfo.status == 2 && animReachMinimumDuration(animRunningInfo, j2))) {
                    animRunningInfo.stop(isAnimEnd && !animRunningInfo.updateToDate().isCanceled);
