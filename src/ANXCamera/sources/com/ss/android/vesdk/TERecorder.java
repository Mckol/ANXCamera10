...
public class TERecorder implements a, AudioDataProcessThread.OnProcessDataListener {
    ...
    public TERecorder(@NonNull String str, @NonNull Context context, @Nullable Handler handler) {
        ...
        this.mGLCreateCallback = new Common.IOnOpenGLCallback() {
            ...
            private float[] fetchTransformMatrixFromTexture(SurfaceTexture surfaceTexture) {
            ...
            private double getDrawFrameTime(long j) {
                ...
                return ((double) (nanoTime - Math.min(Math.min(Math.abs(nanoTime - j), Build.VERSION.SDK_INT >= 17 ? Math.abs(SystemClock.elapsedRealtimeNanos() - j) : Long.MAX_VALUE), Math.abs((SystemClock.uptimeMillis() * ExtraTextUtils.MB) - j)))) / 1000000.0d;
            }
        ...
        this.mNativeInitListener = new NativeInitListener() {
            ...
            public void onNativeInitHardEncoderRetCallback(int i, int i2) {
                boolean z = true;
                if (TERecorder.this.mExternalListenerInvokerHandler != null) {
                    Handler handler = TERecorder.this.mExternalListenerInvokerHandler;
                    VEListener.VERecorderNativeInitListener vERecorderNativeInitListener = TERecorder.this.mExternalNativeInitListener;
                    if (i == 0) {
                        z = false;
                    }
                    handler.post(new NativeHardEncoderInitListenerInvoker(vERecorderNativeInitListener, z));
                } else if (TERecorder.this.mExternalNativeInitListener != null) {
                    VEListener.VERecorderNativeInitListener vERecorderNativeInitListener2 = TERecorder.this.mExternalNativeInitListener;
                    if (i == 0) {
                        z = false;
                    }
                    vERecorderNativeInitListener2.onHardEncoderInit(z);
                }
            }
        };
    ...
    /* access modifiers changed from: private */
    /* access modifiers changed from: public */
    private int getRotation(VECameraSettings.CAMERA_FACING_ID camera_facing_id) {
    ...
    private void startAudioRecorderIfNeed(float f, int i) {
        if ((i & 1) != 0) {
            this.mAudioRecorder.a((double) f, true);
        }
    }
        ...
        this.mThreadPool.execute(new Runnable() {
            ...
            public void run() {
                ...
                if (concat < 0) {
                    ...
                    VELogUtil.e(str, "NativeInvoker concat failed. ret = " + concat);
                }
                if (TERecorder.this.mExternalListenerInvokerHandler != null) {
    ...
    @Override // d.a.a.a, com.ss.android.medialib.audio.AudioDataProcessThread.OnProcessDataListener
