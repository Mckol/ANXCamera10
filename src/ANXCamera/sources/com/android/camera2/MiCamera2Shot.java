...
public abstract class MiCamera2Shot<T> {
    ...
    protected Consumer<Boolean> mShotBoostParams;
    ...
    public final ParallelTaskData generateParallelTaskData(long j, boolean z) {
        ...
        if (pictureCallback != null) {
            return pictureCallback.onCaptureStart(new ParallelTaskData(this.mMiCamera.getId(), j, this.mMiCamera.getCameraConfigs().getShotType(), this.mMiCamera.getCameraConfigs().getShotPath(), this.mMiCamera.getCameraConfigs().getCaptureTime()), this.mMiCamera.getPictureSize(), isQuickShotAnimation(), z, false, false);
        }
