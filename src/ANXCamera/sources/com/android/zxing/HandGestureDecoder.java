...
public class HandGestureDecoder extends Decoder {
    ...
    HandGestureDecoder() {
        ...
        this.mEnable = true;
        this.mSubjects.toFlowable(BackpressureStrategy.LATEST).observeOn(Schedulers.computation()).map(new c(this)).observeOn(AndroidSchedulers.mainThread()).subscribe(new d(this));
    }
    ...
    private void dumpPreviewImage(PreviewImage previewImage) {
        ...
        try {
            ...
            try {
                ...
                try {
                    Log.e(TAG, "Dump preview Image failed!", e2);
