    ...
    public <T> T getValueByItem(TypeItem<T> typeItem) {
        if (!typeItem.mExpandable) {
            ...
            if (typeItem.asString()) {
                return (T) this.mParentDataItem.getString(str, typeItem.mDefaultValue);
            }
            if (typeItem.asInteger()) {
                return (T) Integer.valueOf(this.mParentDataItem.getInt(str, typeItem.mDefaultValue.intValue()));
            }
            if (typeItem.asBoolean()) {
                return (T) Boolean.valueOf(this.mParentDataItem.getBoolean(str, typeItem.mDefaultValue.booleanValue()));
            }
            return null;
        }
