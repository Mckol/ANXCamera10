...
public class V6EffectCropView extends View implements V6FunctionUI {
    ...
    public V6EffectCropView(Context context, AttributeSet attributeSet) {
        ...
        this.mBorderPaint.setStrokeWidth((float) (c.isPad() ? 4 : 2));
    ...
    private void computeCertenLineCrossPoints(Point point, Point point2) {
        ...
        if (squareOfPoints(point, point2) >= MIN_DIS_FOR_SLOPE) {
            ...
            int i10 = (int) (((float) point.y) - (((float) point.x) * f));
            if (i10 >= 0 && i10 <= height && !isContained(pointArr, 0, i10)) {
                pointArr[i] = new Point(0, i10);
                i++;
            }
            int i11 = (int) (((float) point.y) + (((float) (width - point.x)) * f));
    ...
    private void detectMovingEdges(float f, float f2) {
        ...
        if (this.mIsRect) {
            ...
            if (f2 <= f3 + ((float) i) && rectF.top - ((float) i) <= f2) {
            ...
            if (f <= f4 + ((float) i2) && rectF2.left - ((float) i2) <= f) {
            ...
            float f6 = (((float) i3) + width) * (width + ((float) i3));
            float f7 = f - centerX;
            float f8 = f2 - centerY;
    ...
    private void moveCircle(float f, float f2, float f3, float f4) {
        if (this.mMovingEdges == 16) {
            ...
            float f5 = f - centerX;
            float f6 = f2 - centerY;
            float min = Math.min(Math.min(this.mDisplayBounds.width(), this.mDisplayBounds.height()) / 2.0f, Math.max((float) (MIN_CROP_WIDTH_HEIGHT / 2), (float) Math.sqrt((double) ((f5 * f5) + (f6 * f6)))));
