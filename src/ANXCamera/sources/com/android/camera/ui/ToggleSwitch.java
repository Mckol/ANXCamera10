...
public class ToggleSwitch extends CompoundButton implements Checkable {
    ...
    private float[] compThumbRoundRectAttr(float f) {
        ...
        float f3 = (((float) (i + i2)) * (1.0f - f)) + (((((float) i) / 2.0f) + f2) * f);
        float f4 = (float) (i + i2);
        float f5 = (((float) (height - (i * 2))) + f4) - ((float) (i2 * 2));
        return new float[]{f3, f4, ((f2 - ((float) i2)) - ((((float) i) * 3.0f) / 2.0f)) + f3, f5, (f5 - f4) * 0.5f};
    }
    ...
    private void drawRoundRectStroke(Canvas canvas, int i) {
        if (this.mStrokeWidth > 0) {
            ...
            RectF rectF = new RectF();
            rectF.set(f, f2, ((float) width) - f, f3);
