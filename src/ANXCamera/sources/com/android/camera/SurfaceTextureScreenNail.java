...
public abstract class SurfaceTextureScreenNail implements ScreenNail, SurfaceTexture.OnFrameAvailableListener, Rotatable {
    ...
    public interface ExternalFrameProcessor {
        ...
        default void onDrawFrame(Rect rect, int i, int i2, boolean z) {
        }

        default void onDrawFrame(GLCanvasImpl gLCanvasImpl, CameraScreenNail cameraScreenNail) {
        }
    ...
    private void initializePreviewTexture() {
        ...
        if (c.tn() && !sFrameListener.isAlive()) {
        ...
        if (Build.VERSION.SDK_INT < 21 || !c.tn()) {
