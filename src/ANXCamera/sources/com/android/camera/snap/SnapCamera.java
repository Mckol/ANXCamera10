...
public class SnapCamera implements MediaRecorder.OnErrorListener, MediaRecorder.OnInfoListener {
    ...
    private static final String TAG = "SnapCamera";
    ...
    private final CameraCaptureSession.CaptureCallback mCaptureCallback = new CameraCaptureSession.CaptureCallback() {
        ...
        private void process(CaptureResult captureResult) {
            if (SnapCamera.this.mCameraDevice != null && !SnapCamera.this.mFocused && captureResult.get(CaptureResult.CONTROL_AF_STATE) != null) {
                ...
                String str = SnapCamera.TAG;
                Log.d(str, "process: afState=" + num + " aeState=" + captureResult.get(CaptureResult.CONTROL_AE_STATE) + " mFocused=" + SnapCamera.this.mFocused);
                if (2 == num.intValue()) {
    ...
    private void initOrientationListener() {
        this.mOrientationListener = new OrientationEventListener(this.mContext, c.Hm() ? 2 : 3) {
    ...
    private void setupMediaRecorder() {
        ...
        if (VideoModule.VIDEO_MAX_SINGLE_FILE_SIZE < availableSpace && DataRepository.dataItemFeature().Ti()) {
        ...
        if (this.mMediaRecorder != null) {
            if (this.mRecording) {
                try {
                    ...
                    Log.w(TAG, "mMediaRecorder stop failed", e3);
                }
            }
                    ...
                    if (length > 0) {
                        if (!Storage.isUseDocumentMode()) {
                            ...
                            try {
                                ...
                                try {
                                    e2.printStackTrace();
                                    Log.e(TAG, "Failed to write MediaStore " + e2);
                                    Util.closeSilently(parcelFileDescriptor);
                                    if (this.mStatusListener != null) {
                                    }
                                    this.mRecording = false;
            ...
            this.mCameraDevice.createCaptureSession(asList, new CameraCaptureSession.StateCallback() {
                ...
                public void onConfigured(@NonNull CameraCaptureSession cameraCaptureSession) {
                    try {
                            ...
                            public void onCaptureStarted(CameraCaptureSession cameraCaptureSession, CaptureRequest captureRequest, long j, long j2) {
                                if (!SnapCamera.this.mRecording) {
                                    try {
                                        ...
                                        String str = SnapCamera.TAG;
                                        Log.e(str, "failed to start media recorder: " + e2.getMessage(), e2);
                                        e2.printStackTrace();
                                        SnapCamera.this.stopCamcorder();
                                    }
                        ...
                        String str = SnapCamera.TAG;
                        Log.e(str, "videoSessionCb::onConfigured: " + e2.getMessage(), e2);
                    }
                }
            }, this.mCameraHandler);
        } catch (CameraAccessException e2) {
