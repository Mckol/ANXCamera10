...
public class CameraBrightness implements CameraBrightnessCallback {
    ...
    public static class CameraBrightnessTask extends AsyncTask<Void, Void, Integer> {
        ...
        private int[] getAndroidArrayRes(String str) {
            try {
                ...
                try {
                    return CameraAppImpl.getAndroidContext().getResources().getIntArray(GeneralUtils.miuiResArrayField(str, Field.INT_SIGNATURE_PRIMITIVE).getInt(null));
        ...
        private boolean getAndroidBoolRes(String str, boolean z) {
            try {
                ...
                Log.e(CameraBrightness.TAG, e2.getMessage());
                try {
                    return CameraAppImpl.getAndroidContext().getResources().getBoolean(GeneralUtils.miuiResBoolField(str, Field.INT_SIGNATURE_PRIMITIVE).getInt(null));
                } catch (NoSuchFieldException e3) {
                    Log.e(CameraBrightness.TAG, e3.getMessage());
                    return z;
                } catch (IllegalArgumentException e4) {
        ...
        private Integer getBrightIsAndroidP(WindowManager.LayoutParams layoutParams, Activity activity) {
            ...
            if (DataRepository.dataItemFeature().Pk() && cameraBrightnessCallback != null) {
        ...
        private Integer getBrightNotAndroidP(WindowManager.LayoutParams layoutParams) {
            ...
            if (f > 0.0f) {
                ...
                if (Math.abs((spline != null ? Math.round(spline.interpolate(f2)) : Math.round(f2)) - currentBackLight) <= 1) {
        ...
        public float interpolate(float f) {
            ...
            while (true) {
                ...
                if (f < fArr2[i3]) {
                    ...
                    float f4 = 2.0f * f3;
                    float[] fArr4 = this.mM;
                    float f5 = (fArr3[i] * (f4 + 1.0f)) + (fArr4[i] * f2 * f3);
                    float f6 = 1.0f - f3;
                    return (f5 * f6 * f6) + (((fArr3[i3] * (3.0f - f4)) + (f2 * fArr4[i3] * (f3 - 1.0f))) * f3 * f3);
                } else if (f == fArr2[i3]) {
                    return this.mY[i3];
                } else {
                    i = i3;
    ...
    private void adjustBrightness() {
        if (this.mCurrentActivity != null && c.xm()) {
            cancelLastTask();
