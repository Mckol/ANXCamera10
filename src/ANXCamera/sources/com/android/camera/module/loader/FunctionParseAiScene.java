...
public class FunctionParseAiScene implements Function<CaptureResult, Integer> {
    ...
    private final boolean mSupportMoonMode = DataRepository.dataItemFeature().Tj();
        ...
        if (this.mModuleIndex == 171 || CameraSettings.isFrontCamera() || faceArr == null || faceArr.length <= 0) {
            ...
            for (Face face : faceArr) {
                if (((float) face.getBounds().width()) > 300.0f / width) {
                    Log.c(TAG, "parseAiSceneResult: AI_SCENE_MODE_HUMAN  face.length = " + faceArr.length + ";face.width = " + face.getBounds().width() + ";hdrMode = " + CaptureResultParser.getHdrDetectedScene(captureResult));
                    i = 25;
                }
            }
        }
