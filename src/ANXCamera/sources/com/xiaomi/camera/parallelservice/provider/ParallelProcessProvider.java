    ...
    private Cursor queryProcessingMetadata(@Nullable Long l) {
        ...
        for (SaveTask saveTask : this.dbItemSaveTask.getAllItems()) {
            if (z || saveTask.getMediaStoreId() == l) {
                matrixCursor.addRow(new Object[]{saveTask.getMediaStoreId(), saveTask.getPath(), Integer.valueOf(ProcessingMetadataQuery.ProgressStatus.INDETERMINATE.getIdentifier()), Integer.valueOf(saveTask.getPercentage()), saveTask.getStartTime(), Integer.valueOf(saveTask.getNoGaussian()), 0});
            }
        }
